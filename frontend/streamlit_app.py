...
def main():
    ...
    with st.sidebar:
        ...
        agent_options = {
            ...
            "performance": st.checkbox("Performance Agent", value=True, key="agent_performance"),
            "style": st.checkbox("Style Agent", value=True, key="agent_style"),
            "best_practices": st.checkbox("Best Practices Agent", value=True, key="agent_best_practices"),
            "documentation": st.checkbox("Documentation Agent", value=True, key="agent_documentation")
        }
        ...
        with st.spinner("Reviewing code... This may take a few minutes."):
            try:
                result = st.session_state.orchestrator.review(
                    code=code,
                    language=language,
                    include_agents=selected_agents
                )
...
def render_review_results():
        ...
        with st.expander("Syntax Analysis", expanded=True):
            ...
            if syntax.get("syntax_valid"):
                ...
                for issue in syntax.get("issues", [])[:10]:
                    st.write(f"**Line {issue.get('line', '?')}:** {issue.get('message', 'Unknown error')}")
        ...
        with st.expander("Security Analysis", expanded=True):
            ...
            if vulns:
                ...
                for vuln in vulns[:10]:
                    ...
                    st.write(f"{severity_color} **{severity.upper()}:** {vuln.get('message', 'Unknown')}")
                    if vuln.get("line"):
    ...
    # Performance
    if "performance" in agent_results and not agent_results["performance"].get("skipped"):
        with st.expander("Performance Analysis"):
            ...
            if issues:
                for issue in issues[:10]:
                    ...
                    st.write(f"**{impact.upper()} Impact:** {issue.get('message', 'Unknown')}")
                    if issue.get("line"):
        ...
        with st.expander("Style Review"):
            ...
            if issues:
                for issue in issues[:10]:
                    ...
                    st.write(f"**{severity.upper()}:** {issue.get('message', 'Unknown')}")
                    if issue.get("line"):
                        st.caption(f"Line {issue['line']}")
            else:
    ...
    # Best Practices
    if "best_practices" in agent_results and not agent_results["best_practices"].get("skipped"):
        with st.expander("Best Practices Review"):
            ...
            if issues:
                for issue in issues[:10]:
                    st.write(f"**{issue.get('category', 'General')}:** {issue.get('message', 'Unknown')}")
                    if issue.get("line"):
                        st.caption(f"Line {issue['line']}")
            else:
    ...
    # Documentation
    if "documentation" in agent_results and not agent_results["documentation"].get("skipped"):
        with st.expander("Documentation Review"):
            ...
            if issues:
                for issue in issues[:10]:
                    st.write(f"**{issue.get('severity', 'medium').upper()}:** {issue.get('message', 'Unknown')}")
                    if issue.get("line"):
                        st.caption(f"Line {issue['line']}")
            else:
...
def render_report():
    ...
    report = report_generator.generate_report(result, format_type=format_map[report_format])
