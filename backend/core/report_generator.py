...
class ReportGenerator:
    ...
    def generate_report(
        self, review_results: Dict[str, Any], format_type: str = "markdown"
    ) -> str:
        """
    ...
    def _generate_markdown_report(self, results: Dict[str, Any]) -> str:
        ...
        report.append(
            f"- **Overall Severity:** {summary.get('overall_severity', 'none')}"
        )
        ...
        if metrics:
            ...
            report.append(
                f"- **Average Line Length:** {metrics.get('average_line_length', 0):.1f}"
            )
            report.append("")
        ...
        if "security" in agent_results:
            ...
            if not security.get("skipped") and not security.get("error"):
                ...
                report.append("")
                report.append(
                    f"**Security Score:** {security.get('security_score', 0):.1f}/10"
                )
                report.append(
                    f"**Vulnerabilities Found:** {security.get('vulnerability_count', 0)}"
                )
                report.append("")
                ...
                if vulns:
                    for vuln in vulns[:10]:
                        ...
                        report.append(
                            f"{severity_marker} **{severity.upper()}:** {vuln.get('message', 'Unknown')}"
                        )
                        if vuln.get("line"):
        ...
        if "performance" in agent_results:
            ...
            if not performance.get("skipped") and not performance.get("error"):
                ...
                report.append("")
                report.append(
                    f"**Performance Score:** {performance.get('performance_score', 0):.1f}/10"
                )
        ...
        if "best_practices" in agent_results:
            ...
            if not best_practices.get("skipped") and not best_practices.get("error"):
                ...
                report.append("")
                report.append(
                    f"**Best Practices Score:** {best_practices.get('best_practices_score', 0):.1f}/10"
                )
                report.append(
                    f"**Issues Found:** {best_practices.get('issue_count', 0)}"
                )
                report.append("")
        ...
        if "documentation" in agent_results:
            ...
            if not documentation.get("skipped") and not documentation.get("error"):
                ...
                report.append("")
                report.append(
                    f"**Documentation Score:** {documentation.get('documentation_score', 0):.1f}/10"
                )
                ...
                if doc_stats:
                    ...
                    report.append(
                        f"  - Functions with docstrings: {doc_stats.get('functions_with_docstrings', 0)}/{doc_stats.get('total_functions', 0)}"
                    )
                    report.append(
                        f"  - Classes with docstrings: {doc_stats.get('classes_with_docstrings', 0)}/{doc_stats.get('total_classes', 0)}"
                    )

                report.append(
                    f"**Issues Found:** {documentation.get('issue_count', 0)}"
                )
                report.append("")
        ...
        report.append("---")
        report.append("")
        report.append("*Report generated by Code Review Agent System*")
